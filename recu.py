...
def cles(root="keys"):
    ...
    return {"signer": "local-v0", "suite": "ed25519", "note": "not QUANTUM. not a mint. keys off Git.", "pk_sha256": hashlib.sha256(sk.public_key().public_bytes_raw()).hexdigest()}
...
def ecrire(montant, devise, rail, de_, vers, ref=None):
    ...
    return {"format": FORMAT, "recu_id": "RC-" + uuid.uuid4().hex[:12], "montant_cents": cents, "devise": devise, "rail": rail, "de": de_, "vers": vers, "ref_externe": ref, "suite": "ed25519", "pose_at": _now(), "sceau": None, "note": "attestation, pas un solde. FAMILLE ne tient pas l'argent."}
...
def juger(carte):
    ...
    return {"decision": "allow", "flag": "recu", "rail": carte["rail"], "montant_cents": carte["montant_cents"], "devise": carte["devise"], "note": "le reçu tient. l'argent, s'il a bougé, a bougé ailleurs."}
